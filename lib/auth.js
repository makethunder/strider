...
var registerRoutes = function(app){
  ...
  app.get('/unlock', function(req, res, next) {
    return res.render('unlock.html', {});
  })

  app.post('/unlock', function(req, res, next) {
    unlock(req, res, next);
  })
...
var unlock = function(req, res, next){
  var email = req.body.email;
  var password = req.body.password;
  var new_password = req.body.new_password;
  User.changePasswordAndUnlock(email, password, new_password, function(err, user){
    if (!err) {
      res.redirect('/');
    } else {
      console.error("Failed to unlock account with email: %s", email);
      res.redirect('/unlock');
    }
  })
}
