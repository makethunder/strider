...
// information needed to kick off a job

// type: TEST_ONLY or TEST_AND_DEPLOY
// user: the owner of the project?
// project: the project object
// trigger: trigger specification
// done(err, job)
function startJob(type, user, project, trigger, done) {
  var data = {
    type: type,
    user_id: user._id,
  ...
  new Job(data).save(function (err, job) {
    if (err) return done(err)
    data.id = job._id.toString()
    data.project = project.toJson()
    data.provider = project.provider.toJSON()
    data.userProvider = user.providers[data.provider.id]
    // TODO: use a specific runner here
    common.emitter.emit('job.new', data)
