...
UserSchema.static('authenticate', function(email, password, callback) {
  this.findOne({ email: email }, function(err, user) {
    ...
    user.verifyPassword(password, function(err, passwordCorrect) {
      if (err) { return callback(err); }
      if (!passwordCorrect) { return callback("Incorrect PWD"); }
      if (user.get('locked')) { return callback("Account is locked"); }
...
UserSchema.static('changePasswordAndUnlock', function(email, password, new_password, cb){
  this.findOne({ email: email }, function(err, user) {
    user.verifyPassword(password, function(err, ok) {
      if (err) { return cb(err); }
      if (!ok) { return cb("Incorrect PWD"); }
      if (password == new_password) { return cb("New password must not be the same."); }
      if (new_password === undefined || new_password === '' ) { return cb("No empty passwords."); }
      user.set('password', new_password);
      user.set('locked', false);
      user.save(function(err, user){
        if (err)
          return cb("Error Saving User:" + err);
