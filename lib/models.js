...
var ProjectSchema = new Schema({
  ...
  // TODO: make webhooks a separate plugin
...
ProjectConfig.virtual('has_prod_deploy_target').get(function() {
...
JobSchema.index({ 'ref.branch': -1 }, { sparse: true });
...
var UserSchema = new Schema({
  ...
  // defined by the individual provider plugins
  , providers: {}
  // map of "projectname": AccessLevel
  , projects: {}

  /*** Backwards compat. TODO remove ***/
  /*
  , github_config: [ProjectConfig]
  , github_metadata: {}
  , github: GithubSchema
  , heroku: [HerokuConfig]
...
UserSchema.methods.get_repo_config = function(case_insensitive_url, cb) {
  ...
  console.debug("user.get_repo_config() - no repos found matching %s in %j, querying DB",
    ...
    User.findOne({"github_config":{$elemMatch:{
      ...
      console.error(
        "user.get_repo_config() - DANGER WILL ROBINSON. This should never happen. Project is private and user is not a collaborator. How could this be?");
