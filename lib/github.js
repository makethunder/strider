    ...
    function loop(uri, page) {
      get_oauth2(uri, {per_page:30, page:page}, access_token, function(error, response, body) {
        if (!error && response.statusCode == 200) {
          ...
          // Stop condition: No link header or no more next link
...
var verify_webhook_req_signature = exports.verify_webhook_req_signature = function(req, callback)
{
  ...
  if (payload.repository === undefined) {
    console.log("verify_webhook_req_signature() payload.repository is undefined!  payload: %s", payload);
    callback(false);
    return;
  }
...
var webhook_extract_latest_commit_info = exports.webhook_extract_latest_commit_info = function(payload)
{
    ...
    // Get branch name from the ref field which looks something like: "ref":"refs/heads/master"
    var pattern = new RegExp("[aA-zZ]+/[aA-zZ]+/", "i");
    var branch = payload.ref.replace(pattern, "");
    ...
    return {
      branch: branch,
      id: commit_data && commit_data.id,
      author: commit_data && commit_data.author,
      message: commit_data && commit_data.message,
      timestamp: commit_data && commit_data.timestamp
    };
};
