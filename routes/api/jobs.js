...
exports.jobs_start = function (req, res) {
    ...
    var found = _.find(
        supported_types, function (ttype) {
    ...
    req.user.get_repo_config(
        url, function (err, repo_config, access_level, origin_user_obj) {
            if (err || !repo_config) {
                var msg = "error getting repo config: " + err + ", repo_config: " + repo_config;
                console.log(msg);
                res.statusCode = 400;
                return res.end(
                    JSON.stringify(
            ...
            if (origin_user_obj.github.id) {
                repo_metadata = _.find(
                    origin_user_obj.github_metadata[origin_user_obj.github.id].repos, function (item) {
            ...
            if (job_type === TEST_ONLY) {
                return jobs.startJob(
                    req.user, repo_config, deploy_config, github_commit_info, repo_ssh_url, job_type, function (job) {
            ...
            if (!repo_config.has_prod_deploy_target) {
                return jobs.startJob(
                    req.user, repo_config, undefined, github_commit_info, repo_ssh_url, job_type, function (job) {
                        res.end(JSON.stringify({job: job}));
                    }
                );
            }
            ...
            var deploy_config = _.find(
                req.user[deploy_config_key], function (item) {
...
exports.repo_jobs = function (req, res) {
    ...
    ljobs.lookup(
        repo_url, function (err, repo_config) {
            Job.find()
...
exports.jobs = function (req, res) {
    ...
    Step(
        ...
        function buildQueries(err, repo_list) {
            ...
            _.each(
                this.repo_list, function (configured_repo) {
                    Job.find()
        ...
        function processAndRender(err, results) {
            ...
            _.each(
                results, function (jobl) {
                    ...
                    var repo_config = _.find(
                        repo_list, function (config) {
                    ...
                    if (repo_config === undefined) {
                        console.debug(
                            "jobs() - job object exists for repo %s but no config found in repo list: %j",
            ...
            l.sort(
                function (a, b) {
                    if (!a.finished_timestamp || !a.finished_timestamp.getTime) {
                        return true;
                    }
                    if (!b.finished_timestamp || !b.finished_timestamp.getTime) {
                        return false;
                    }
