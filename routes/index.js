...
exports.index = function(req, res){
  ...
  if (req.session && req.session.return_to) {
...
exports.webhook_signature = function(req, res)
{
  gh.verify_webhook_req_signature(req, function(isOk, repo, user, payload) {
    var active = isOk && repo && (repo.active === undefined? true : repo.active) || false;
    
    if (active) {
      console.log("received webhook payload for repo '%s'; starting task on behalf of %s", repo.url, user.email);
      handleWebhook(repo, user, payload);
      res.end("webhook good");
    } else {
      console.log("received an incorrecly signed webhook");
...
/*
 * POST /webhook?:secret - Github push webhook handler (manual repo setup case)
 */
